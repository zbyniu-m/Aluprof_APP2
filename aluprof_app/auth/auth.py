...
from aluprof_app.models import user_schema, User, UserSchema, user_password_update_schema
from aluprof_app.utils import validate_json_content_type, token_required
...
def register(args: dict):
    ...
    if User.query.filter(User.email == args['email']).first():
        abort(409, description=f'User with username {args["email"]} already exists')

    args['password'] = User.generate_hasgerd_passwort(args['password'])
    user = User(**args)
...
def update_user_password(user_id: int, args: dict):
    ...
    user.password = user.generate_hasgerd_passwort(args['new_password'])
    db.session.commit()
